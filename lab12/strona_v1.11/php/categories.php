...
class Categories {
    ...
    function DodajKategorie() {
        ...
        // Dodaj kategorię główną tylko jeśli tabela jest pusta
        $check_empty = "SELECT COUNT(*) as count FROM kategorie";
        $empty_result = $conn->query($check_empty);
        $row_count = $empty_result->fetch_assoc()['count'];
        ...
        // Wyświetl wybór kategorii nadrzędnej tylko jeśli istnieją kategorie
        if(!empty($categories)) {
            echo '<div class="form-group">';
        ...
        if (isset($_POST['submit_category'])) {
            ...
            try {
                // Jeśli tabela jest pusta, dodaj pierwszą kategorię z matka=0
                if($row_count == 0) {
                    // Dla pierwszej kategorii najpierw dodajemy ją samą jako root
                    $insert_query = "INSERT INTO kategorie (id, nazwa, matka) VALUES (1, '$name', 0)";
                } else {
                    $parent = intval($_POST['parent_category']);
                    // Sprawdź czy rodzic istnieje
                    $check_parent = "SELECT id FROM kategorie WHERE id = $parent";
                    $parent_result = $conn->query($check_parent);
                    
                    if($parent_result->num_rows > 0 || $parent === 0) {
                        $insert_query = "INSERT INTO kategorie (nazwa, matka) VALUES ('$name', $parent)";
                    } else {
                ...
                echo '<div class="error-message">Błąd podczas dodawania kategorii: ' . $e->getMessage() . '</div>';
            }
        }
    ...
    function EdytujKategorie() {
        ...
        if (isset($_POST['submit_edit_category'])) {
            ...
            $update_query = "UPDATE kategorie SET nazwa = '$new_name', matka = $new_parent WHERE id = $category_id";
        ...
        $query = "SELECT id, nazwa, matka FROM kategorie ORDER BY nazwa ASC";
        $result = $conn->query($query);
        $categories = array();
        $categories[0] = "Brak (kategoria główna)";
        $category_list = array();
        ...
        if (isset($_GET['edit_category']) || isset($_POST['edit_category_select'])) {
            ...
            $edit_query = "SELECT * FROM kategorie WHERE id = $edit_id";
    ...
    function UsunKategorie() {
        ...
        if (isset($_POST['submit_delete_category'])) {
            ...
            $check_children_query = "SELECT COUNT(*) as child_count FROM kategorie WHERE matka = $category_id";
            ...
            if ($child_row['child_count'] > 0) {
                ...
                // Usuń kategorię
                $delete_query = "DELETE FROM kategorie WHERE id = $category_id";
